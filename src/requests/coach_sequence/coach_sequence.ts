...
export default async function coachSequence(lineNumber: string, time: string): Promise<CoachSequence> {
  const reponse = await fetch(`https://www.apps-bahn.de/wr/wagenreihung/1.0/${lineNumber}/${time}`, {
    method: "GET",
    next: {
      revalidate: 90
    }
